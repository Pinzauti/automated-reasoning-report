...
\end{minted}
this will solve the problem with the input data you should enter in the file \file{input.lp} located in the directory \file{src\\asp\\data}. The \mintinline{bash}{s} argument is the same as explained above. All of this is managed by the \mintinline{python}{asp_manual()} function in the \file{main.py} file.
...
If you want to use manual mode you should insert the input data in the file \file{input.dzn} located in the directory \file{src\\minizinc\\data} and run the following command:
\begin{minted}{bash}
...
\subsubsection{Both}
You can also solve both the ASP and the MiniZinc problems at the same time, inserting the input data in the file \file{input.lp} located in the directory \file{src\\asp\\data} and \file{input.dzn} located in the directory \file{src\\minizinc\\data}. The command is
\begin{minted}{bash}
