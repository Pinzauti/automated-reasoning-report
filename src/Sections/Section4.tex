...
In the first one we require each path to do the prescribed number of turns, in the second one we impose that the number of intersection point (i.e. point labelled as \mintinline[breaklines]{minizinc}{number + 1}) is exactly one. Then with the last one we impose the number of starting cells (i.e. labelled with 12 in the first dimension of the board) to be exactly the same as the number of starting points, this constraint could seem redundant, but for how the predicates in section~\ref{subsec:predicates} were defined it turned to be necessary.
